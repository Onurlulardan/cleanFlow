...
namespace cleanFlow.Repositories.AssignRepository
{
    public class AssignRepository : IAssignRepository
    {
        ...
        {
            string query = "SELECT A.*, B.NAME, B.SURNAME, C.WCTYPE, C.MAHALCODE, C.LOCATIONID, WL.LOCATIONNAME FROM ASSIGN A LEFT JOIN PERSONELS B ON A.PERSONELID = B.PERSONELID LEFT JOIN WC C ON A.WCID = C.WCID LEFT JOIN LOCATIONS WL ON C.LOCATIONID = WL.LOCATIONID WHERE A.PERSONELID = @PERSONELID";
